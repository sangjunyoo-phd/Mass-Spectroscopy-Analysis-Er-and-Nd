...
def get_data_conc():     # Return or generate global variables of Extraction result (1,N)
    path = 'extraction_results/'
    files = os.listdir(path)    # List of excel file names in the path
    ...
    for file in files:
        if file.endswith('.csv') == True:   # Read csv files only
            ...
            # Assign element columns with boolean values
...
def curve_peleg(x,k1,k2):
    return x/(1/k1+x/k2)

def curve_exp(x,c,k):
    return c*(1-np.exp(-x*k))

def curve_exp_modified(x,c,k,b):
    return c*(1-np.exp(b-x*k))

def curve_2exp(x,c_1,k_1,c_2,k_2):
    return c_1*(1-np.exp(-x*k_1)) + c_2*(1-np.exp(-x*k_2))

def curve_lin_to_exp(x,c,k,b):
    return c
